...
# Optional: Serper (search) + ScrapeWebsite only when SERPER_API_KEY is set.
# Wrappers supply default search_query/url when Bedrock doesn't pass them (avoids "search_query is required").
def _security_analyst_tools():
    if os.getenv("SERPER_API_KEY") and os.getenv("SERPER_API_KEY").strip() not in ("", "add-your-serper-api-key"):
        from aws_infrastructure_security_audit_and_reporting.tools.search_tools_with_defaults import (
            SerperSearchWithDefaults,
            ScrapeWebsiteWithDefaults,
        )
        return [SerperSearchWithDefaults(), ScrapeWebsiteWithDefaults()]
    return []  # No web search; analyst uses only Task 1 context + LLM knowledge
