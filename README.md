...
**Option 1 – CrewAI CLI (from project root):**
```bash
crewai run
```

**Option 2 – Python module (from project root):**
```bash
# Run the crew
PYTHONPATH=src python3 -m aws_infrastructure_security_audit_and_reporting.main run

# Other commands:
# PYTHONPATH=src python3 -m aws_infrastructure_security_audit_and_reporting.main train <n_iterations> <filename>
# PYTHONPATH=src python3 -m aws_infrastructure_security_audit_and_reporting.main replay <task_id>
# PYTHONPATH=src python3 -m aws_infrastructure_security_audit_and_reporting.main test <n_iterations> <openai_model_name>
```

**Option 3 – From inside `src` (so the package is on path):**
```bash
cd src
python3 -m aws_infrastructure_security_audit_and_reporting.main run
```

This initializes the AWS Infrastructure Security Audit crew and produces a report (e.g. `report.md` in the project root).
